...
typedef struct _PARTITION_TABLE {
	...
	PARTITION_ENTRY Partitions[14];
    UCHAR Reserved2[0x90];
} PARTITION_TABLE, *PPARTITION_TABLE;

typedef struct _LBA48_PATCH {
    ULONG Magic[2];
    ULONG PatchCodeVersion;
    ULONG LowCodeBase;
    ULONG HighCodeBase;
    ULONG PatchSegSize;
    PPARTITION_TABLE PartitionTable;
} LBA48_PATCH, *PLBA48_PATCH;
...
extern "C"
{
	...
	extern void WINAPI KeQuerySystemTime (PLARGE_INTEGER CurrentTime);
	extern NTSTATUS NTAPI NtSetSystemTime (PLARGE_INTEGER SystemTime, PLARGE_INTEGER PreviousTime OPTIONAL);
	extern NTSYSAPI BOOLEAN NTAPI RtlTimeFieldsToTime(TIME_FIELDS* TimeFields, PLARGE_INTEGER Time);
